...
import pydfmux.spt3g.northern_tuning_script_anb as nts
...
###### user parameters ######
logfile = '/home/spt3g/he10_logs/20171019_read.h5'
setpoints = np.linspace(0.250, 0.500, 9)
wafertemps_filename = '/home/spt3g/output/20171031/GofT_temps.pkl'

###### THIS IS THE PART WHERE IT DOES THINGS ########
print 'Starting G(T)'

sc.ChaseLS.set_PID_temp(1, 0.00)
...
sc.He3UCp.set_voltage(2.3)
print 'Heating UC pump a little for overbias'
print('{:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()))
time.sleep(300)
sc.ChaseLS.set_PID_temp(1, 0.500)
time.sleep(1)
sc.ChaseLS.set_heater_range(2)
while float(gt.gettemp(logfile, 'UC Stage'))<0.490:
    time.sleep(10)
#sc.He3UCp.set_voltage(3)
...
#    sc.He3ICp.set_voltage(0)
    ...
    while float(gt.gettemp(logfile, 'UC Stage'))>(setpoints[jtemp]+0.01):
	time.sleep(60)
    ...
    while np.abs(recenttemps[-1] - recenttemps[-4]) > 0.001 and nAttempts < 45:
        ...
        nAttempts += 1
        print('{:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()))
        print('Wafer holder drifted %5.2f mK.' % (1e3*np.abs(recenttemps[-1] - recenttemps[-4])))
    if nAttempts == 45:
        sc.ChaseLS.set_heater_range(0)
        sys.exit('UC Head failed to stabilize! Zeroed heater and quitting now.')
    print 'Temperature Stabilized, Proceeding'
    ...
    nts.run_take_noise()
    nts.run_drop_bolos()
    #drop_bolos_results =  bolos.drop_bolos(monotonic=True,A_STEP_SIZE=0.000015,TOLERANCE=0.03)
